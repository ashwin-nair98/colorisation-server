...
var destroy_editor = function() {
    var editor = editors.pop();
    var elem = document.getElementById(editor.config.name);
    elem.removeChild(editor.view.ctx.canvas)
}
var img = new Image();
...
Editor.prototype = {
  ...
  render: function() {
    ...
    v.frame("process_button", 461 - 39, 148, 82, 40, () => {
      if (this.progress != null) {
        v.ctx.font = "12px Verdana"
        v.ctx.fillStyle = "#000"
        var s = "Downloading"
    ...
    v.frame("upload_button", 192-32, 310, 64, 40, () => {
      if (do_button(v, "Upload")) {
...
function do_button(v, text) {
  ...
  if (request_in_progress) {
    animate(name, parse_color("#424242FF"), 100)
  } else if (mouse_down && v.contains(mouse_pos)) {
    animate(name, parse_color("#115293FF"), 50)
  } else {
    if (v.contains(mouse_pos)) {
      animate(name, parse_color("#4791dbFF"), 100)
    } else {
      animate(name, parse_color("#1976d2FF"), 100)
    }
  }
